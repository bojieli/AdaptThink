...
MODEL_PATH_LIST=("deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B" "deepseek-ai/DeepSeek-R1-Distill-Qwen-7B")
for MODEL_PATH in "${MODEL_PATH_LIST[@]}"; do
    CKPT_DIR=/mnt/zjj/open-source/github/AdaptThink/ckpts/adapt_think_verl/${MODEL_PATH##*/}
    RESULT_SAVE_DIR=$CKPT_DIR/eval_HF_results_len${max_response_length}_n${rollout}
    EXP_NAME="eval-len${max_response_length}-n${rollout}-${MODEL_PATH##*/}"
